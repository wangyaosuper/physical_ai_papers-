...
> 本文整理了 15 篇近期热门 Physical AI 论文,按研究方向归类,并做简要介绍。
...
- **热度**: 33
- **背景**: 车端世界模型对未来的预测与驾驶任务的端到端规划之间存在明显脱节——要么预测表征在预训练后冻结、无法随规划目标自适应进化;要么多个轨迹候选共享同一个未来潜变量,导致打分器无法区分「同样几何形状但不同安全后果」的轨迹,最终"预测得好≠决策得好"。
- **核心方案**: 从表征层、预测层、打分层三层联动实现「决策对齐」:
  1. **JEPA 驱动的预测表征自适应**: 采用「在线编码器(LoRA 适配 V-JEPA)+ EMA 动量目标编码器」双塔结构,在整个规划器优化过程中**持续施加 JEPA 预测监督**,使未来潜变量空间与打分目标协同进化,而非预训练后冻结不变。
  2. **动作条件化的反事实未来建模(一一对应)**: 为 **N 个轨迹候选中的每一个**生成**独立专属的未来潜变量**(而非多候选共享同一个未来)。共享预测器通过显式的「场景表征 × 轨迹动作」交互模块,为每个候选输出其动作执行后对应的特定未来后果 Ẑ_i。
  3. **未来潜变量条件化的因子化打分器**: Scorer 同时输入「当前场景 Z_t + 轨迹动作 a_i + 该轨迹专属未来潜变量 Ẑ_i」三元组,采用因子化解码同时输出**可解释规划因子**(碰撞/车道偏离/违规等)和**整体效用分**,直接用候选专属的未来后果指导打分排序。
  4. **专家匹配 + 安全关键硬负样本的监督策略**: 离线日志只有专家轨迹有真实未来观测,因此先从候选集中找到与专家轨迹最匹配的那一个,仅对它施加密集 JEPA 潜变量预测损失;同时构造「几何上与专家极相似、但安全结果不同」的硬负样本,在规划决策边界附近提供局部对比监督,防止 Scorer 只看几何形状不看未来后果。
- **关键突破**: 明确了"预测好≠决策好"的核心痛点,通过「每候选一个专属未来潜变量 + 三元组联合打分 + 全程 JEPA 自适应表征」的三层联动,从机制上解决了「预测与决策解耦」「多候选未来混淆」两大顽疾,在 NAVSIM-v1/v2 上取得 SOTA。
- **推理范式**: 部署时仅保留在线编码器 + 动作条件预测器 + 因子化打分器,直接为所有候选生成专属未来并排序。
...
### 13. WA-JEPA: Rethinking the Video JEPA Paradigm for World-Action Modeling in Autonomous Driving
- **Paper ID**: 2608.20974
- **作者**: Xinlin Wang, Yujiao Xiang, Yuheng Zhou
- **热度**: 34
- **背景**: 传统世界模型 V-JEPA 基于纯视觉预测,直接用于自动驾驶领域存在诸多问题,纯视觉信息并没有抓取出驾驶的本质特征。
- **创新点**: 本论文在 V-JEPA 基础上做了诸多优化,包括训练阶段对外来画面的掩码方式,引入基于条件流匹配(condition flow matching)的隐空间形式,对未来场景和自车轨迹进行联合预测,将模型能力更加聚焦于驾驶的本质特征。该论文的重要价值在于证明视觉模型(如 JEPA)可以改造为强大的因果动作预测器。
- **链接**: https://arxiv.org/html/2608.20974v1
...
| JEPA 物理状态锚定 | Is Forward Prediction Enough?、WA-JEPA | 训练时锚定物理状态/将 V-JEPA 改造为因果动作预测器,推理时零成本提升表征与决策质量 |
